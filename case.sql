...
SELECT
    ...
    , t.NumberOfPatients
    , t.NumberOfPatientsInSurgery
    , FORMAT(100.0 * t.NumberOfPatientsInSurgery / t.NumberOfPatients, 'N2') AS PercentageOfPatientsInSurgery
...
-- version from copilot
    SELECT 
        ...
        , COUNT(*) AS NumberOfPatients
        , SUM(CASE WHEN ps.Ward LIKE '%Surgery' THEN 1 ELSE 0 END) AS NumberOfPatientsInSurgery
        , FORMAT(100.0 * SUM(CASE WHEN ps.Ward LIKE '%Surgery' THEN 1 ELSE 0 END) / COUNT(*), 'N2') AS PercentageOfPatientsInSurgery
    FROM
        dbo.PatientStay ps
    GROUP BY 
        ps.Hospital
